...
function buildPrompt(input: PlannerInput): string {
  ...
  const destinationInstruction = isSurpriseMe
    ? `The traveler has NOT picked a destination — they want you to choose one for them ("Surprise me"). Pick ONE specific, real, well-known travel destination that realistically fits their starting point ("${input.startLocation}"), budget (INR ${input.budget}), trip length (${input.days} days), interests (${input.interests.join(", ") || "general sightseeing"}), and travel style (${input.travelStyle}). Prefer destinations reachable from the starting point within the given budget and days. Use this chosen destination's real name as the "destination" field in your response — never write "Surprise me" in the output.`
...
- Include realistic travel/transit details for getting from "${input.startLocation}" to the destination, and back to "${input.endLocation}" (flights, trains, buses, shared taxis, etc. as appropriate for the real route) as part of Day 1 and the final day's activities.
...
{
  "destination": string, // the real destination name (and country, if outside India) — REQUIRED even in surprise-me mode
...
- Destination: ${isSurpriseMe ? "(you choose — see instructions above)" : input.destination}
